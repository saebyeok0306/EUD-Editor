...
const MemoizedListItem = React.memo(({ item, isActive, onClick }) => (
  ...
  >
    <div style={{ 
      ...
      <FlingyPreview flingyId={item.id} />
    </div>
...
function FlingyTab({ mapData, datReady }) {
  const { t } = useI18n()
  ...
  useEffect(() => {
    ...
    const names = []
    const keys = Object.keys(flingyData)

    for (let i = 0; i < keys.length; i++) {
      const id = parseInt(keys[i], 10)
      names.push({ id, name: `Flingy ${id}` })
    }
    setFlingyNames(names)
  }, [datReady])
  ...
  const currentItemData = (currentFlingyData && selectedItem !== null) ? currentFlingyData[selectedItem] : null
  const spriteId = currentItemData?.['Sprite']
  ...
  ), [flingyNames, selectedItem, listWidth])
      ...
      <div className="properties-pane">
        {selectedItem !== null ? (
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--ev-c-divider)', paddingBottom: '15px', marginBottom: '15px' }}>
              ...
              <div>
                <h3 style={{ margin: '0 0 10px 0', color: 'var(--ev-c-text-1)', fontSize: '18px' }}>
                  {flingyNames.find(i => i.id === selectedItem)?.name || `Flingy ${selectedItem}`}
                </h3>
          ...
          <div className="properties-empty">
            ...
            <div className="properties-empty-text">{t('unit.empty.text', { defaultValue: 'Select a flingy from the list' })}</div>
          </div>
        )}
      </div>
    </div>
