...
function TechTab({ mapData, datReady }) {
  ...
  useEffect(() => {
    ...
    const names = []
    const keys = Object.keys(techdataData)

    for (let i = 0; i < keys.length; i++) {
      const id = parseInt(keys[i], 10)
      const labelIdx = techdataData[id]['Label']
      let name = `Tech ${id}`
      ...
      const iconIdx = techdataData[id]['Icon']
      ...
      <div className="properties-pane">
        {selectedItem !== null ? (
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--ev-c-divider)', paddingBottom: '15px', marginBottom: '15px' }}>
              ...
              <div>
                <h3 style={{ margin: '0 0 10px 0', color: 'var(--ev-c-text-1)', fontSize: '18px' }}>
                  {techNames.find(i => i.id === selectedItem)?.name || `Tech ${selectedItem}`}
                </h3>
          ...
          <div className="properties-empty">
            ...
            <div className="properties-empty-text">{t('unit.empty.text', { defaultValue: 'Select a tech from the list' })}</div>
          </div>
        )}
      </div>
    </div>
