...
function UpgradeTab({ mapData, datReady }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      const labelIdx = upgradesData[id]['Label']
      let name = `Upgrade ${id}`
      ...
      const iconIdx = upgradesData[id]['Icon']
  ...
  ), [upgradeNames, selectedItem, listWidth])
      ...
      <div className="properties-pane">
        {selectedItem !== null ? (
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--ev-c-divider)', paddingBottom: '15px', marginBottom: '15px' }}>
              ...
              <div>
                <h3 style={{ margin: '0 0 10px 0', color: 'var(--ev-c-text-1)', fontSize: '18px' }}>
                  {upgradeNames.find(i => i.id === selectedItem)?.name || `Upgrade ${selectedItem}`}
                </h3>
          ...
          <div className="properties-empty">
            ...
            <div className="properties-empty-text">{t('unit.empty.text', { defaultValue: 'Select an upgrade from the list' })}</div>
          </div>
        )}
      </div>
    </div>
