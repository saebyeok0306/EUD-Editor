...
const unitsDefPath = path.resolve('src/renderer/src/dat/units.def');
const unitsDatPath = path.resolve('src/renderer/src/dat/units.dat');

function main() {
  console.log('Loading files...');
  const defText = fs.readFileSync(unitsDefPath, 'utf8');
  const datBuf = fs.readFileSync(unitsDatPath);
  
  const defData = parseDef(defText);
  const units = parseDat(datBuf.buffer.slice(datBuf.byteOffset, datBuf.byteOffset + datBuf.byteLength), defData);
  
  console.log('--- Unit Target Acquisition Range (Sample) ---');
  // Print first 15 units
  for (let i = 0; i < 15; i++) {
    const u = units[i];
    console.log(`ID ${String(i).padStart(3, '0')}: ${u['Target Acquisition Range']}`);
  }

  console.log('\n--- Units with non-zero Target Acquisition Range ---');
  const nonZero = units.filter(u => u['Target Acquisition Range'] > 0);
  console.log(`Found ${nonZero.length} units.`);
  nonZero.forEach(u => {
    console.log(`ID ${String(u.id).padStart(3, '0')}: ${u['Target Acquisition Range']}`);
  });
}
